...
# Function to add edges between the entities in the graph 
def add_edge(n1, n2):
...
def load_movie_data():
    ...
    with open(movies_directors_filename, "r") as fin:
        ...
        for line in fin:
            ...
            if 'm' + str(m_id) in movies:
                movies["m"+str(m_id)].director = director
    ...
    with open(movies_actors_filename, "r") as fin:
        ...
        for line in fin:
            ...
            if 'm' + str(m_id) in movies:
                movies["m"+str(m_id)].actors.append(actor)
            actors.add(actor)
    ...
    with open(movies_genres_filename, "r") as fin:
        ...
        for line in fin:
            ...
            if 'm' + str(m_id) in movies:
                movies["m"+str(m_id)].genres.append(genre)
            genres.add(genre)
...
def records_to_graph():
	...
	j = 1
        while j <= 5:
            v = str(i)+'_'+str(j)	           
            if v not in nodedict.keys():
                n = Node(counter,v,'rating')
                nodelist.append(n)
                nodedict[v] = counter
                counter = counter + 1
            j = j + 1
    ...
    for d in directors:
        n = Node(counter,d,'director')
        nodelist.append(n)
        nodedict[d] = counter
        counter = counter + 1

    for g in genres:
        n = Node(counter,g,'genre')
        nodelist.append(n)
        nodedict[g] = counter
        counter = counter + 1

    for a in actors:
        n = Node(counter,a,'actor')
        nodelist.append(n)
        nodedict[a] = counter
        counter = counter + 1
    ...
    for k in movies.keys():
        ...
        if d is not None:
		    i_d = int(nodedict[d])
			i_k = int(nodedict[k])
            add_edge(nodelist[i_d],nodelist[i_k])
			#nodelist[i_d].neighbors.append(nodelist[i_k])
            #nodelist[i_k].neighbors.append(nodelist[i_d])      
        for g in movies[k].genres:
		    i_g = int(nodedict[g])
			i_k = int(nodedict[k])
            add_edge(nodelist[i_g],nodelist[i_k])
			#nodelist[i_g].neighbors.append(nodelist[i_k])
            #nodelist[i_k].neighbors.append(nodelist[i_g])
        for a in movies[k].actors:
		    i_a = int(nodedict[a])
			i_k = int(nodedict[k])
			add_edge(nodelist[i_a],nodelist[i_k])
            #nodelist[i_a].neighbors.append(nodelist[i_k])
            #nodelist[i_k].neighbors.append(nodelist[i_a])      
        
    edge_list = []

    for k in ratings.keys():
        for r in ratings[k].keys():
            v = str(r)+'_'+str(ratings[k][r])
			i_v = int(nodedict[v])
			i_k = int(nodedict[k])
			add_edge(nodelist[i_v],nodelist[i_k])
			#nodelist[i_v].neighbors.append(nodelist[i_k])
            #nodelist[i_k].neighbors.append(nodelist[i_v])            
            if v not in edge_list:
                edge_list.append(v)
				i_r = int(nodedict[r])
				add_edge(nodelist[i_v],nodelist[i_r])
                #nodelist[i_v].neighbors.append(nodelist[i_r])
                #nodelist[i_r].neighbors.append(nodelist[i_v])            
