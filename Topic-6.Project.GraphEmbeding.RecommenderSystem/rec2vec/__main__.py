...
def process(args):
    ...
    walk = graph.build_deepwalk_corpus(G,no_walks,walk_len)
    model = Word2Vec(walk,min_count = 0,workers = args.workers, size = args.representation_size, window = args.window_size)       
...
def predict_rating(model, nodedict, user, movie):
    ...
    v = movie+'_'+str(1)
    usr = str(nodedict[user])
    init = model.similarity(str(nodedict[v]),usr)
    rat_val = 1
    i = 2
    while i <= 5:
        v = movie+'_'+str(i) 
        sim_val = model.similarity(str(nodedict[v]),usr)
        if init > sim_val:
           rat_val = i
           init = sim_val
        i=i+1
