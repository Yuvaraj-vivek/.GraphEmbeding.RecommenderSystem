...
def records_to_graph():
    ...
    for dctr in directors:
        n = Node(counter,dctr,'director')
        nodelist.append(n)
        nodedict[dctr] = counter
        counter = counter + 1

    for actr in actors:
        n = Node(counter,actr,'actor')
        nodelist.append(n)
        nodedict[actr] = counter
        counter = counter + 1

    for gnr in genres:
        n = Node(counter,gnr,'genre')
        nodelist.append(n)
        nodedict[gnr] = counter
        counter = counter + 1

    for ky in ratings.keys():
        n = Node(counter,ky)
        nodelist.append(n)
        nodedict[ky] = counter
        counter = counter + 1
    ...
    for ky in movies.keys():
    	dtr = movies[ky].director
        if dtr is not None:
        	i_d = int(nodedict[dtr])
		i_k = int(nodedict[ky])
                add_edge(nodelist[i_d],nodelist[i_k])
		
        for act in movies[ky].actors:
        	i_a = int(nodedict[act])
		i_k = int(nodedict[ky])
		add_edge(nodelist[i_a],nodelist[i_k])

        for gnre in movies[ky].genres:
        	i_g = int(nodedict[gnre])
		i_k = int(nodedict[ky])
    ...
    for ky in ratings.keys():
    	for r in ratings[ky].keys():
        	v = str(r)+'_'+str(ratings[ky][r])
	        i_v = int(nodedict[v])
	        i_k = int(nodedict[ky])
