...
def process(args):
    ...
    model = Word2Vec(walk,min_count = 0,workers = args.workers, size = arg.representation_size, window = args.window_size)       
...
def predict_rating(model, nodedict, user, movie):
	...
	while i <= 5:
	    v = movie+'_'+str(i)
        mid = nodedict[v].id
        sim_val = model.similarity(uid, mid)
        if sim_val > max_val:
            max_val = sim_val
            rat_val = i
		i = i+1
